...
public class UI : MonoBehaviour 
{
	...
	public void onLoginFailed(UInt16 failedcode, byte[] serverData)
	{
        if (failedcode == 20)
		{
            err("login is failed(登陆失败), err=" + KBEngineApp.app.serverErr(failedcode) + ", " + System.Text.Encoding.ASCII.GetString(serverData));
        }
		else
		{
